...
* getCreateFields() 获取的创建字段(一维数组),返回promise，一般情况下该函数的this指向create组件实例
...
* doCreateRequest(data) data是经过transformData处理过的表单数据，需要返回promise表示已完成。一般情况下该函数的this指向create组件实例
